...
def test_detect_loc(structural_smell_detector, tmp_path):
    ...
    assert any(
        "High Lines of Code (LOC)" in smell.name
        for smell in structural_smell_detector.structural_smells
    )
...
def test_detect_noc(structural_smell_detector, tmp_path):
    ...
    structural_smell_detector.detect_smells(str(tmp_path))
    assert any(
        "High Number of Classes (NOC)" in smell.name
        for smell in structural_smell_detector.structural_smells
    )


def test_pathspec_excludes_structural_file(structural_smell_detector, tmp_path):
    big_module = tmp_path / "big_module.py"
    big_module.write_text("\n".join([f"print({i})" for i in range(1001)]))

    pathspec = compile_pathspec(["big_module.py"])
    structural_smell_detector.detect_smells(str(tmp_path), pathspec)
