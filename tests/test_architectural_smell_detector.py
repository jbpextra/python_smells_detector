...
from code_quality_analyzer.path_filters import compile_pathspec
...
def test_detect_god_object(architectural_smell_detector, tmp_path):
    ...
    assert any(
        "God Object" in smell.name for smell in architectural_smell_detector.architectural_smells
    )
...
def test_detect_scattered_functionality(architectural_smell_detector, tmp_path):
    ...
    architectural_smell_detector.detect_smells(str(tmp_path))
    assert any(
        "Scattered Functionality" in smell.name
...
def test_detect_redundant_abstraction(architectural_smell_detector, tmp_path):
    ...
    architectural_smell_detector.detect_smells(str(tmp_path))
    assert any(
        "Redundant Abstraction" in smell.name
        for smell in architectural_smell_detector.architectural_smells
    )
...
def test_detect_improper_api_usage(architectural_smell_detector, tmp_path):
    ...
    architectural_smell_detector.detect_smells(str(tmp_path))
    assert any(
        "Improper API Usage" in smell.name
        for smell in architectural_smell_detector.architectural_smells
    )


def test_pathspec_excludes_architectural_file(architectural_smell_detector, tmp_path):
    god_object = tmp_path / "god_object.py"
    god_object.write_text("\n".join([f"def func{i}(): pass" for i in range(30)]))

    pathspec = compile_pathspec(["god_object.py"])
    architectural_smell_detector.detect_smells(str(tmp_path), pathspec)
